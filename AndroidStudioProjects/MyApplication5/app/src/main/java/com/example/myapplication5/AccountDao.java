...
//db.execSQl("insert into account(name,blance )values(?.?)new Object[]{?,?}))")
//db.execSql("delete form account where_id=?
//db.excesql(update account set name=?  where_id=?)
//Cursor cursor=db.rawQuery( select* from account  where    orderBy balance  )
