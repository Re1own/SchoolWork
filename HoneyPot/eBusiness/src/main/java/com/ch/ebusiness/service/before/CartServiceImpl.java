...
public class CartServiceImpl implements CartService{
	...
	public String putCart(Goods goods, Model model, HttpSession session) {
		...
		good = goodsRepository.selectAGoods(id);	//goodtable中的good,goods是carttable中的
		...
		if(cartRepository.isPutCart(uid, goods.getId()).size() > 0) {
			...
			for (Map<String, Object> map : list) {
				if ((Integer)map.get("id") == good.getId()) {
					if (goods.getBuyNumber() + (Integer)map.get("shoppingnum") > good.getGstore()) {
						return "peach";
					}
					else {
...
//		model.addAttribute("orderDetail", cartRepository.orderDetail(id));
		List<Map<String, Object>> list = cartRepository.orderDetail(id);
